...
void main ()
{
	...
	HWIFI_VoidSendHttpRequest((uint8*)"http://fota-nc.freevar.com/upgrade.txt", (uint8*)"44");
	...
	else if(Global_uint8Record[0] == UPDATE)
	{
		...
		while(Global_uint8Record[8] != HEX_END)
		{
			...
			HWIFI_voidConnectToServer();
			/*Sending HTTP Request To the Server*/
			HWIFI_VoidSendHttpRequest((uint8*)"http://fota-nc.freevar.com/Recive.php?response=OK", (uint8*)"55");
			
			/*Reconnecting the Wifi To the Server*/
			HWIFI_voidConnectToServer();
			/*Sending HTTP Request To the Server*/
			HWIFI_VoidSendHttpRequest((uint8*)"http://fota-nc.freevar.com/code.txt", (uint8*)"41");
